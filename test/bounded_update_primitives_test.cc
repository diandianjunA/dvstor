...
void test_stage2_admission_depends_only_on_fixed_context_capacity() {
  using memory_node_storage_owner_maintenance_detail::
    Stage2AdmissionDecision;
  using memory_node_storage_owner_maintenance_detail::
    decide_stage2_admission;

  assert(decide_stage2_admission(true, false) ==
         Stage2AdmissionDecision::unavailable);
  assert(decide_stage2_admission(false, true) ==
         Stage2AdmissionDecision::unavailable);
...
int main() {
  ...
  test_stage2_admission_depends_only_on_fixed_context_capacity();
