...
RUN_ROOT="${RUN_ROOT:-$PROGRAM_DIR/results/program3_story_$(date +%Y%m%d_%H%M%S)}"
ACTION="${1:-all}"

mkdir -p "$RUN_ROOT"
MANIFEST="$RUN_ROOT/story_manifest.tsv"
if [[ ! -f "$MANIFEST" ]]; then
...
wait_storage() {
  ...
  echo "请先在存储节点运行："
  echo "  cd $PROJECT_DIR"
  echo "  GPU_COMMIT_WIDTH=$width ./motivation/program3/start_storage_case.sh $mode"
...
print(f"[program3-story] valid kind={kind} mode={mode} width={width}")
PY
}
...
summarize() {
  python3 "$PROGRAM_DIR/summarize_story.py" "$RUN_ROOT"
  python3 "$PROGRAM_DIR/plot_story.py" "$RUN_ROOT"
  echo "结果目录：$RUN_ROOT"
}
