...
RESULT_ROOT="${RESULT_ROOT:-$MOTIVATION_DIR/results/feedback_pricing}"
...
sample_rate_for_concurrency() {
  case "$1" in
    1) printf '%s\n' "${TRACE_SAMPLE_RATE_C1:-20}" ;;
    64) printf '%s\n' "${TRACE_SAMPLE_RATE_C64:-500}" ;;
    *) printf '%s\n' "${TRACE_SAMPLE_RATE_C256:-2000}" ;;
  esac
}
...
for concurrency in $CONCURRENCIES; do
  for repetition in $(seq 1 "$REPETITIONS"); do
    ...
    trace_path="$run_dir/beam_turnover.jsonl"
    mkdir -p "$run_dir"
    (
      set -a
      source "$MOTIVATION_DIR/configs/common.env"
      source "$MOTIVATION_DIR/configs/feedback_pricing.env"
    ...
    "$MOTIVATION_DIR/analyze_feedback_pricing_motivation.py" \
      "$trace_path" \
      --baseline-json "$BASELINE_JSON" \
      --output-json "$run_dir/analysis.json" \
      --output-markdown "$run_dir/analysis.md"
  done
done
