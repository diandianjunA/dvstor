...
- `build/dvstor_memory_node`: storage/memory-node-only service
- `build/vamana_offline_builder`: offline Vamana builder that exports DVSTOR shard files plus RaBitQ artifacts

Storage nodes only need RDMA dependencies. On machines without CUDA, GPUs, or DOCA GPUNetIO, build only the storage
node artifact:
```bash
cmake -S . -B build-storage -DCMAKE_BUILD_TYPE=Release -DDVSTOR_STORAGE_NODE_ONLY=ON
cmake --build build-storage -j
```
This produces `build-storage/dvstor_memory_node` and skips CUDA, GPU kernels, compute service, tests, and GPUNetIO.
