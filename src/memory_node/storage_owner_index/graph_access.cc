...
bool MemoryNode::read_node_snapshot(RemotePtr rptr, NodeSnapshot& snapshot) {
  ...
  for (u32 attempt = 0; attempt < kMaxReadAttempts; ++attempt) {
    ...
    if (!parse_remote_snapshot(rptr, read_buffer, candidate)) {
      std::this_thread::yield();
      continue;
    }
    u64 after = 0;
...
vec<MemoryNode::NodeSnapshot> MemoryNode::read_node_snapshots_batched(const vec<RemotePtr>& rptrs,
  ...
  for (size_t begin = 0; begin < rptrs.size(); begin += max_batch) {
    ...
    const size_t first_remote_snapshot = snapshots.size();
    vec<PendingRead> valid_pending;
...
void MemoryNode::write_graph_adjacency(
    ...
    std::optional<u32> generation_override,
    std::optional<bool> deleted_override) {
  if (!VamanaNode::hot_graph_entry_available(rptr)) return;
...
void MemoryNode::write_new_node_on_shard(
  ...
  *reinterpret_cast<u32*>(
  ...
  const u8 stable_count = static_cast<u8>(
    std::min<size_t>(neighbors.size(), VamanaNode::R));
  VamanaNode::encode_hot_graph_entry(
    record.data() + VamanaNode::HOT_GRAPH_DYNAMIC_HOT_OFFSET,
    stable_count, neighbors.data(), stable_count,
