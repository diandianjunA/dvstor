...
inline bool storage_owner_batch_prefers_sync_local_stitch(
    const configuration::IndexConfiguration& config,
    const vec<service::storage_owner::MutationKind>& kinds,
    const vec<u64>& anchor_hints, u32 anchor_hint_count, size_t item_count) {
  if (!storage_owner_local_stitch_mode(config) ||
      !config.storage_owner_local_stitch_sync_fast_path ||
      anchor_hint_count == 0) {
    return false;
  }
  if (anchor_hints.size() <
      item_count * static_cast<size_t>(anchor_hint_count)) {
    return false;
  }
