...
struct GpuNetioQueryPool::Resource {
  explicit Resource(const configuration::IndexConfiguration& config,
                    ...
                    const size_t persistent_data_bytes = 0,
                    const uint32_t qps_per_node = 1)
      : dim(config.dim),
    ...
    const size_t scratch_bytes =
      ...
      10 * sizeof(uint64_t) + 256 + persistent_data_bytes + kGpuPageSize;
    ...
    if (persistent_data_bytes > 0) {
      persistent_data = static_cast<unsigned char*>(allocate(persistent_data_bytes, kGpuPageSize));
      persistent_data_size = persistent_data_bytes;
    }
...
struct GpuNetioPersistentTransport::Impl {
  Impl(const configuration::IndexConfiguration& config,
    ...
    resource = std::make_unique<GpuNetioQueryPool::Resource>(
      config, 0, context, cm, remote_regions, data_bytes, qps_per_node);
  }

  u32 qps_per_node{};
  std::unique_ptr<GpuNetioQueryPool::Resource> resource;
};
...
GpuNetioPersistentTransport::GpuNetioPersistentTransport(
    ...
    ClientConnectionManager& cm,
    const MemoryRegionTokens& remote_regions)
    : impl_(std::make_unique<Impl>(config, data_bytes, context, cm, remote_regions)) {}
...
GpuNetioPersistentView GpuNetioPersistentTransport::view() const {
  const auto& resource = *impl_->resource;
  return {
    .qp_array = resource.d_qp_array,
    .remote_regions = resource.d_remote_regions,
