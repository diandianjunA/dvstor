...
void atomic_max_query(std::atomic<u64>& value, u64 candidate) {
  ...
  while (candidate > observed && !value.compare_exchange_weak(
...
HostOrchestratedSearchEngine::Impl::route_seeds(
  ...
  const auto home = centroid_home::select_published_snapshot(
    query, routes.home);
...
void HostOrchestratedSearchEngine::Impl::score_candidates(
  ...
  for (size_t candidate = 0; candidate < candidates.size(); ++candidate) {
    ...
    if (!host_orchestrated_policy::resolve_record(
          index, candidates[candidate].pointer, storage_region_bytes,
          record)) {
      continue;
    }
    ...
    std::memset(lane.packed_dynamic_codes.data() +
    ...
    if (!record.immutable_base) {
      dynamic_reads.push_back({
        ...
        .local_offset = dynamic_code_scratch_offset +
          candidate * dynamic_code_record_bytes,
  ...
  if (!dynamic_reads.empty()) {
    ...
    engine.telemetry_.dynamic_code_wait_ns.fetch_add(
      std::chrono::duration_cast<std::chrono::nanoseconds>(
        Clock::now() - wait_started).count(), std::memory_order_relaxed);
    engine.telemetry_.dynamic_code_reads.fetch_add(
      dynamic_reads.size(), std::memory_order_relaxed);
  ...
  for (size_t candidate = 0; candidate < candidates.size(); ++candidate) {
    ...
    if (!record.immutable_base) {
      const byte_t* dynamic_record = lane.scratch +
        dynamic_code_scratch_offset + candidate * dynamic_code_record_bytes;
      ...
      if (output != candidate) {
        std::memset(lane.packed_dynamic_codes.data() + output * code_bytes,
                    0, code_bytes);
      }
    }
  ...
  check_lane_cuda(lane, cudaMemcpyAsync(
    lane.d_lut, lane.lut.data(), lane.lut.size() * sizeof(f32),
    cudaMemcpyHostToDevice, lane.stream), "cudaMemcpyAsync(host PQ LUT)");
  check_lane_cuda(lane, cudaMemcpyAsync(
    lane.d_ordinals, lane.ordinals.data(),
    candidates.size() * sizeof(u32), cudaMemcpyHostToDevice, lane.stream),
    "cudaMemcpyAsync(host PQ ordinals)");
  check_lane_cuda(lane, cudaMemcpyAsync(
    lane.d_dynamic_codes, lane.packed_dynamic_codes.data(),
    candidates.size() * code_bytes, cudaMemcpyHostToDevice, lane.stream),
    "cudaMemcpyAsync(host dynamic PQ codes)");
  host_distance::launch_pq(
    lane.stream, d_base_codes, lane.d_ordinals, lane.d_dynamic_codes,
    static_cast<u32>(candidates.size()), code_bytes, lane.d_lut,
    lane.d_distances);
  check_lane_cuda(lane, cudaGetLastError(), "host PQ distance launch");
  check_lane_cuda(lane, cudaMemcpyAsync(
    lane.distances.data(), lane.d_distances,
    candidates.size() * sizeof(f32), cudaMemcpyDeviceToHost, lane.stream),
  ...
  const u64 elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(
    Clock::now() - started).count();
  const u64 cuda_elapsed =
    std::chrono::duration_cast<std::chrono::nanoseconds>(
      Clock::now() - cuda_started).count();
  engine.telemetry_.gpu_score_ns.fetch_add(elapsed,
                                            std::memory_order_relaxed);
  engine.telemetry_.gpu_pq_score_ns.fetch_add(
    cuda_elapsed, std::memory_order_relaxed);
...
void HostOrchestratedSearchEngine::Impl::fetch_graph_wave(
  ...
  for (size_t item = 0; item < wave.size(); ++item) {
    ...
    const u32 bytes = adaptive
      ? graph_record_validation::graph_extent_bytes_for_class(
    ...
    if (!record.immutable_base) {
      engine.telemetry_.dynamic_graph_read_bytes.fetch_add(
        bytes, std::memory_order_relaxed);
      (states[item].partial
        ? engine.telemetry_.dynamic_graph_short_reads
        : engine.telemetry_.dynamic_graph_full_reads).fetch_add(
          1, std::memory_order_relaxed);
    }
    (states[item].partial
      ? engine.telemetry_.graph_live_extent_reads
      : engine.telemetry_.graph_full_record_reads).fetch_add(
        1, std::memory_order_relaxed);
  }
  read_batch(lane, reads, true);
  engine.telemetry_.graph_page_requests.fetch_add(
  ...
  for (u32 attempt = 0; attempt < maximum_batch_attempts; ++attempt) {
    ...
    for (size_t item = 0; item < wave.size(); ++item) {
      ...
      const byte_t* record = lane.scratch + graph_scratch_offset +
        item * graph_entry_bytes;
      const auto snapshot = state.partial
      ...
      if (snapshot == graph_record_validation::SnapshotState::valid) {
        ...
        if (adaptive && graph_record_validation::required_live_extent_bytes(
          ...
          } else if (state.record.immutable_base &&
                     state.started_partial && !state.partial) {
    ...
    validation_ns +=
      std::chrono::duration_cast<std::chrono::nanoseconds>(
        Clock::now() - validation_started).count();
    ...
    engine.telemetry_.graph_shard_batches.fetch_add(
      ...
      std::memory_order_relaxed);
    engine.telemetry_.graph_page_requests.fetch_add(
      retry.size(), std::memory_order_relaxed);
    engine.telemetry_.critical_graph_reads.fetch_add(
      retry.size(), std::memory_order_relaxed);
  }
  ...
  engine.telemetry_.gpu_neighbor_decode_ns.fetch_add(
    std::chrono::duration_cast<std::chrono::nanoseconds>(
      Clock::now() - decode_started).count(), std::memory_order_relaxed);
  const u64 elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(
    Clock::now() - started).count();
  engine.telemetry_.gpu_graph_ns.fetch_add(elapsed,
                                            std::memory_order_relaxed);
}
...
service::QueryResult HostOrchestratedSearchEngine::Impl::exact_rerank(
  ...
  for (size_t candidate = 0; candidate < attempted; ++candidate) {
    ...
    const size_t local = exact_scratch_offset +
      output * exact_record_stride;
  ...
  const u64 exact_shard_batches = populated_shards<ReadRequest>(
    records, index.shards.size());
  ...
  engine.telemetry_.exact_snapshot_train_fallbacks.fetch_add(
    exact_shard_batches, std::memory_order_relaxed);
  engine.telemetry_.exact_vector_reads.fetch_add(
  ...
  for (size_t record = 0; record < records.size(); ++record) {
    u64 header_after = 0;
    std::memcpy(&header_after,
                lane.scratch + exact_header_scratch_offset +
                  record * sizeof(u64),
                sizeof(header_after));
    const byte_t* source = lane.scratch + exact_scratch_offset +
      record * exact_record_stride;
    if (!host_orchestrated_policy::exact_snapshot_visible(
          source, exact_record_bytes, header_after, handles[record])) {
      continue;
    }
    if (valid != record) {
      std::memmove(lane.scratch + exact_scratch_offset +
                     valid * exact_record_stride,
                   source, exact_record_stride);
      source = lane.scratch + exact_scratch_offset +
        valid * exact_record_stride;
    }
    node_t id = 0;
  ...
  check_lane_cuda(lane, cudaMemcpyAsync(
    lane.d_query, lane.query.data(), lane.query.size() * sizeof(f32),
    cudaMemcpyHostToDevice, lane.stream),
    "cudaMemcpyAsync(host exact query)");
  check_lane_cuda(lane, cudaMemcpyAsync(
    lane.d_exact_records, lane.scratch + exact_scratch_offset,
    valid * exact_record_stride, cudaMemcpyHostToDevice, lane.stream),
    "cudaMemcpyAsync(host exact records)");
  host_distance::launch_exact(
    lane.stream, lane.d_query, lane.d_exact_records,
    static_cast<u32>(valid), config.dim, config.resolved_vector_dtype(),
    exact_record_stride, static_cast<u32>(VamanaNode::offset_vector()),
    lane.d_exact_distances);
  check_lane_cuda(lane, cudaGetLastError(), "host exact distance launch");
  check_lane_cuda(lane, cudaMemcpyAsync(
    lane.distances.data(), lane.d_exact_distances, valid * sizeof(f32),
  ...
  std::sort(exact.begin(), exact.end(), [](const Exact& left,
                                           const Exact& right) {
  ...
  engine.telemetry_.gpu_exact_ns.fetch_add(
    std::chrono::duration_cast<std::chrono::nanoseconds>(
      Clock::now() - started).count(), std::memory_order_relaxed);
  return result;
}
...
service::QueryResult HostOrchestratedSearchEngine::Impl::execute_query(
  ...
  decode_storage_vector_to_float(
    query_data, query_dtype, config.dim, lane.query.data());
  for (f32 value : lane.query) {
    if (!floating_value_is_finite(value)) {
      throw std::invalid_argument("host query components must be finite");
    }
  }
  pq::build_distance_table(
    pq_model, lane.query, lane.lut, lane.transformed);
  engine.telemetry_.gpu_prepare_ns.fetch_add(
    std::chrono::duration_cast<std::chrono::nanoseconds>(
      Clock::now() - prepare_started).count(), std::memory_order_relaxed);
  lane.beam = route_seeds(*routes, lane.query);
  if (!lane.beam.empty()) {
    engine.telemetry_.graph_route_hits.fetch_add(
      1, std::memory_order_relaxed);
  }
  ...
  while (expansions < config.gpu_max_expansions) {
    ...
    const u64 selection_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(
        selection_complete - selection_started).count();
    ...
    engine.telemetry_.gpu_visited_ns.fetch_add(
      std::chrono::duration_cast<std::chrono::nanoseconds>(
        Clock::now() - visited_started).count(),
      std::memory_order_relaxed);
    score_candidates(lane, lane.pending);
    const auto merge_started = Clock::now();
    lane.beam.insert(lane.beam.end(), lane.pending.begin(),
                     lane.pending.end());
    std::sort(lane.beam.begin(), lane.beam.end(), less);
    lane.beam.erase(std::unique(
      lane.beam.begin(), lane.beam.end(), [](const Candidate& left,
    ...
    const u64 merge_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
      Clock::now() - merge_started).count();
    engine.telemetry_.gpu_beam_merge_ns.fetch_add(
      merge_ns, std::memory_order_relaxed);
    engine.telemetry_.gpu_beam_ns.fetch_add(
      merge_ns, std::memory_order_relaxed);
    ...
    engine.telemetry_.issue_width_sum.fetch_add(
      wave.size(), std::memory_order_relaxed);
...
service::QueryResult HostOrchestratedSearchEngine::Impl::search(
  ...
  if (query_data == nullptr || static_cast<u32>(query_dtype) >
      static_cast<u32>(VectorDType::int8) || k == 0 ||
      k > std::max(config.k, config.gpu_final_rerank_width)) {
  ...
  for (u32 route_attempt = 0; route_attempt < 2; ++route_attempt) {
    auto routes = std::atomic_load_explicit(
      &route_snapshot, std::memory_order_acquire);
    if (routes == nullptr) {
      throw std::runtime_error("host centroid routes are unavailable");
    }
    service::QueryResult result = execute_query(
      lane, query_dtype, query_data, k, routes);
    if (!result.empty()) {
      if (!healthy.load(std::memory_order_acquire) ||
          stopping.load(std::memory_order_acquire)) {
        throw std::runtime_error(unhealthy_message());
      }
      engine.telemetry_.queries_completed.fetch_add(
        1, std::memory_order_relaxed);
      engine.telemetry_.batches.fetch_add(1, std::memory_order_relaxed);
      engine.telemetry_.batch_queries.fetch_add(1,
                                                 std::memory_order_relaxed);
      engine.telemetry_.gpu_active_ns.fetch_add(
        std::chrono::duration_cast<std::chrono::nanoseconds>(
          Clock::now() - query_started).count(), std::memory_order_relaxed);
      return result;
    }
