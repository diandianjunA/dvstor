...
void ComputeService::run_storage_insert_cq_loop() {
  ...
  for (;;) {
    const bool progressed = poll_completions();
...
void ComputeService::handle_storage_owner_send_completion(u32 owner_storage, u32 slot_id) {
  ...
  bool queued = false;
  {
    ...
    queued = queue_storage_owner_completion_locked(state, slot);
  }
  if (queued) {
...
bool ComputeService::queue_storage_owner_completion_locked(
  ...
  state.ready_slots.push_back(slot.slot_id);
  return true;
}

void ComputeService::run_storage_owner_completion(u32 owner_storage) {
  auto& state = *storage_insert_owners_[owner_storage];
  for (;;) {
    u32 slot_id = 0;
    {
      ...
      state.completion_cv.wait(lock, [&]() {
        return !state.ready_slots.empty() ||
               storage_insert_completion_done_.load(std::memory_order_acquire);
      });
      if (state.ready_slots.empty()) {
        return;
      }
      slot_id = state.ready_slots.front();
      state.ready_slots.pop_front();
    }

    StorageOwnerPublicationBatch publication;
    commit_storage_owner_slot(owner_storage, slot_id, publication);
    ...
    release_storage_owner_slot(owner_storage, slot_id);
  }
}
...
void ComputeService::release_storage_owner_slot(u32 owner_storage, u32 slot_id) {
  auto& state = *storage_insert_owners_[owner_storage];
  {
    std::lock_guard<std::mutex> lock(state.mutex);
    lib_assert(slot_id < state.slots.size(),
