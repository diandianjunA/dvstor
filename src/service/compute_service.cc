...
u64 storage_owner_wr_id(u32 owner_storage, u32 slot_id) {
...
void ComputeService<Distance>::start_storage_insert_runtime() {
  ...
  for (u32 owner = 0; owner < owner_count; ++owner) {
    ...
    for (u32 slot_id = 0; slot_id < rpc_depth; ++slot_id) {
      ...
      slot.response_region =
        std::make_unique<LocalMemoryRegion>(context_, slot.response_buffer.data(), slot.response_buffer.size());
...
void ComputeService<Distance>::post_storage_owner_batch(
  ...
  qp.post_receive(*slot.response_region, static_cast<u32>(response_size), wr_id);
