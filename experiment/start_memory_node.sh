...
if [[ "$INSERT_EXECUTION" == "storage_owner" ]]; then
  ...
  cmd+=(--storage-owner-maintenance-policy "${STORAGE_OWNER_MAINTENANCE_POLICY:-delta_log}")
  ...
  cmd+=(--storage-owner-maintenance-max-dirty-per-period "${STORAGE_OWNER_MAINTENANCE_MAX_DIRTY_PER_PERIOD:-1}")
  cmd+=(--storage-owner-maintenance-max-expansions "${STORAGE_OWNER_MAINTENANCE_MAX_EXPANSIONS:-4}")
  cmd+=(--storage-owner-maintenance-max-candidates "${STORAGE_OWNER_MAINTENANCE_MAX_CANDIDATES:-192}")
  cmd+=(--storage-owner-maintenance-high-watermark "${STORAGE_OWNER_MAINTENANCE_HIGH_WATERMARK:-0.75}")
  cmd+=(--storage-owner-maintenance-exact-audit-rate "${STORAGE_OWNER_MAINTENANCE_EXACT_AUDIT_RATE:-4096}")
  cmd+=(--storage-owner-maintenance-audit-min-overlap "${STORAGE_OWNER_MAINTENANCE_AUDIT_MIN_OVERLAP:-0.5}")
