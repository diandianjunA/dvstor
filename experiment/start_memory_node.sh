...
if [[ "$INSERT_EXECUTION" == "storage_owner" ]]; then
  ...
  if [[ "$update_mode" == "anchored" ]]; then
    ...
    cmd+=(--storage-owner-route-top-owners "${STORAGE_OWNER_ROUTE_TOP_OWNERS:-2}")
    cmd+=(--storage-owner-route-confidence-threshold "${STORAGE_OWNER_ROUTE_CONFIDENCE_THRESHOLD:-0.10}")
