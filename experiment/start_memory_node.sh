...
if [[ "$INSERT_EXECUTION" == "storage_owner" ]]; then
  ...
  if [[ "$update_mode" == "anchored" ]]; then
    ...
    cmd+=(--storage-owner-anchor-rehome-upsert "${STORAGE_OWNER_ANCHOR_REHOME_UPSERT:-0}")
    cmd+=(--storage-owner-route-top-owners "${STORAGE_OWNER_ROUTE_TOP_OWNERS:-1}")
    cmd+=(--storage-owner-route-confidence-threshold "${STORAGE_OWNER_ROUTE_CONFIDENCE_THRESHOLD:-0.0}")
    cmd+=(--storage-owner-anchor-refresh-interval "${STORAGE_OWNER_ANCHOR_REFRESH_INTERVAL:-100000}")
    cmd+=(--storage-owner-dynamic-anchor-cap "${STORAGE_OWNER_DYNAMIC_ANCHOR_CAP:-64}")
  fi
  cmd+=(--storage-owner-repair-workers "${STORAGE_OWNER_REPAIR_WORKERS:-1}")
  cmd+=(--storage-owner-repair-budget-us "${STORAGE_OWNER_REPAIR_BUDGET_US:-0}")
  cmd+=(--storage-owner-repair-period-us "${STORAGE_OWNER_REPAIR_PERIOD_US:-1000}")
  cmd+=(--storage-owner-repair-queue-depth "${STORAGE_OWNER_REPAIR_QUEUE_DEPTH:-65536}")
  cmd+=(--storage-owner-pending-edge-cap "${STORAGE_OWNER_PENDING_EDGE_CAP:-256}")
