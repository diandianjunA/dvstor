...
EXPERIMENT_DIR="$PROJECT_DIR/experiment/deep100m"
...
wait_storage() {
  ...
  if [[ "$case_name" == quality ]]; then
    echo "  QUALITY_STAGE2_DELAY_MS=$QUALITY_STAGE2_DELAY_MS ./experiment/deep100m/program1/start_storage_case.sh quality"
  else
    echo "  ./experiment/deep100m/program1/start_storage_case.sh $case_name"
  fi
...
run_update_case() {
  ...
  local report
  report="$(find "$report_dir" -type f -name 'deep100m_*.json' \
