...
./experiment/deep100m/program2/run_program2.sh
```

计算节点会依次提示三个命令。在存储节点分别运行：

```bash
./experiment/deep100m/program2/start_storage_case.sh fixed
./experiment/deep100m/program2/start_storage_case.sh header
./experiment/deep100m/program2/start_storage_case.sh live
```
...
```bash
RUN_ROOT=/path/to/program2_TIMESTAMP ./experiment/deep100m/program2/run_program2.sh live
RUN_ROOT=/path/to/program2_TIMESTAMP ./experiment/deep100m/program2/run_program2.sh summarize
```

可选协议 probe（不是主实验必需）：

```bash
RUN_ROOT=/path/to/program2_TIMESTAMP ./experiment/deep100m/program2/run_program2.sh probe
```
