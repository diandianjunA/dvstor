...
# 普通用户只需修改 deep100m_common.sh 的 "Benchmark input files" 配置块，
# 或在命令行用同名环境变量覆盖。
show_help() {
  cat <<'EOF'
Usage: ./experiment/deep100m/run_breakdown.sh [PROFILE]
...
示例：
  WORKLOAD=insert BENCHMARK_CLIENT_THREADS=24 ./experiment/deep100m/run_breakdown.sh 04_gpu_persistent_gpunetio
  WORKLOAD=mixed READ_RATIO=0.5 ./experiment/deep100m/run_breakdown.sh 04_gpu_persistent_gpunetio
  WORKLOAD=mixed MIXED_MODE=rate_limited TARGET_QUERY_QPS=5000 TARGET_WRITE_QPS=1000 \
    ./experiment/deep100m/run_breakdown.sh 04_gpu_persistent_gpunetio
...
if [[ "$prepare_recall_query" == "1" ||
  ...
  PREPARE_BENCHMARK_DATA="$prepare_benchmark_data" \
    "$EXPERIMENT_DIR/prepare_deep100m_data.sh"
fi
...
if (( needs_recall_data )); then
  [[ -s "$RECALL_QUERY_FILE" ]] || {
    ...
    echo "run PREPARE_QUERY=1 ./experiment/deep100m/prepare_deep100m_data.sh once on a data-preparation node" >&2
    exit 1
  }
  [[ -s "$GROUNDTRUTH_PATH" ]] || {
    echo "missing groundtruth file: $GROUNDTRUTH_PATH" >&2
    echo "run PREPARE_GROUNDTRUTH=1 ./experiment/deep100m/prepare_deep100m_data.sh once on a data-preparation node" >&2
    exit 1
  }
fi
...
if (( needs_performance_query )); then
  if [[ ! -s "$PERFORMANCE_QUERY_PATH" ]]; then
    ...
    echo "set PERFORMANCE_QUERY_FILE to a large held-out .fbin query set" >&2
    exit 1
  fi
...
if (( needs_insert_data )) && [[ ! -s "$INSERT_PATH" ]]; then
  ...
  echo "set INSERT_FILE to a held-out .fbin insert set" >&2
  exit 1
fi
...
mkdir -p "$OUT_DIR"
JSON_REPORT="$OUT_DIR/deep100m_${PROFILE}_${TS}.json"
TEXT_REPORT="$OUT_DIR/deep100m_${PROFILE}_${TS}.txt"
