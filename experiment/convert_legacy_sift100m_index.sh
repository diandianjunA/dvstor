...
ensure_built vamana_legacy_index_converter
mkdir -p "$(dirname "$INDEX_PREFIX")"

cmd=("$BUILD_DIR/vamana_legacy_index_converter"
  ...
  --encode-chunk-vectors "${PQ_ENCODE_CHUNK_VECTORS:-32768}"
  --entry-points "${GPU_ENTRY_POINTS:-256}"
  --threads "$BUILD_THREADS"
  --seed "${SEED:-1234}")
if [[ -n "${PQ_REUSE_MODEL:-}" ]]; then cmd+=(--reuse-model "$PQ_REUSE_MODEL"); fi
if [[ "${OVERWRITE_INDEX:-0}" == "1" ]]; then cmd+=(--overwrite); fi

printf '[migrate] command:'; printf ' %q' "${cmd[@]}"; echo
"${cmd[@]}"
