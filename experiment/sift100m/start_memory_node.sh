...
cmd=("$BUILD_DIR/dvstor_memory_node"
  ...
  --gpu-exact-frontier-early-issue "${GPU_EXACT_FRONTIER_EARLY_ISSUE:-false}"
  --gpu-graph-commit-width "$GPU_GRAPH_COMMIT_WIDTH"
  --storage-owner-batch-max "${STORAGE_OWNER_BATCH_MAX:-32}"
  --storage-owner-batch-max-wait-us "${STORAGE_OWNER_BATCH_MAX_WAIT_US:-10000}"
  --storage-owner-stage2-batch-max-wait-us "${STORAGE_OWNER_STAGE2_BATCH_MAX_WAIT_US:-25000}"
  --storage-owner-stage2-initial-delay-ms "${STORAGE_OWNER_STAGE2_INITIAL_DELAY_MS:-0}"
  --storage-owner-stage2-score-many "${STORAGE_OWNER_STAGE2_SCORE_MANY:-true}"
  --storage-owner-stage2-home-rpc-combining "${STORAGE_OWNER_STAGE2_HOME_RPC_COMBINING:-true}"
  --storage-owner-stage2-graph-issue-width "${STORAGE_OWNER_STAGE2_GRAPH_ISSUE_WIDTH:-16}"
  ...
  --storage-owner-maintenance-queue-depth "${STORAGE_OWNER_MAINTENANCE_QUEUE_DEPTH:-65536}"
