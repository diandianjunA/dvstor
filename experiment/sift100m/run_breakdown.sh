...
show_help() {
  cat <<'EOF'
Usage: ./experiment/sift100m/run_breakdown.sh [PROFILE]
...
示例：
  WORKLOAD=insert BENCHMARK_CLIENT_THREADS=24 ./experiment/sift100m/run_breakdown.sh 04_gpu_persistent_gpunetio
  WORKLOAD=mixed READ_RATIO=0.5 ./experiment/sift100m/run_breakdown.sh 04_gpu_persistent_gpunetio
  WORKLOAD=mixed MIXED_MODE=rate_limited TARGET_QUERY_QPS=5000 TARGET_WRITE_QPS=1000 \
    ./experiment/sift100m/run_breakdown.sh 04_gpu_persistent_gpunetio
...
if (( needs_recall_data )); then
  [[ -s "$RECALL_QUERY_FILE" ]] || {
    ...
    echo "run PREPARE_QUERY=1 ./experiment/sift100m/prepare_sift100m_data.sh once on a data-preparation node" >&2
    exit 1
  }
  [[ -s "$GROUNDTRUTH_PATH" ]] || {
    echo "missing groundtruth file: $GROUNDTRUTH_PATH" >&2
    echo "run PREPARE_GROUNDTRUTH=1 ./experiment/sift100m/prepare_sift100m_data.sh once on a data-preparation node" >&2
    exit 1
  }
fi
