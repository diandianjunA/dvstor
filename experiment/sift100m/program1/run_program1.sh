...
wait_storage() {
  ...
  if [[ "$case_name" == quality ]]; then
    echo "  QUALITY_STAGE2_DELAY_MS=$QUALITY_STAGE2_DELAY_MS ./experiment/sift100m/program1/start_storage_case.sh quality"
  else
    echo "  ./experiment/sift100m/program1/start_storage_case.sh $case_name"
  fi
