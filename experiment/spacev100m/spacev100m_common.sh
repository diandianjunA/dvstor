...
DATASET_DIR="${DATASET_DIR:-/data/xjs/datasets/spacev1b/spacev100m}"
WORK_DIR="${WORK_DIR:-/data/xjs/index/dvstor_spacev100m}"
CONVERTED_DIR="${CONVERTED_DIR:-$WORK_DIR/prepared}"
...
K="${K:-10}"
DIM="${DIM:-100}"
VECTOR_DATA_TYPE="${VECTOR_DATA_TYPE:-int8}"
...
validate_build_thread_limits() {
  ...
  for name in BUILD_JOBS BUILD_THREADS; do
    value="${!name}"
    if [[ ! "$value" =~ ^[1-9][0-9]*$ ]] || ((value > BUILD_THREAD_LIMIT)); then
...
PQ_SUBQUANTIZERS="${PQ_SUBQUANTIZERS:-20}"
...
BENCHMARK_VECTOR_SOURCE="${BENCHMARK_VECTOR_SOURCE:-$DATASET_DIR/spacev100m_base.i8bin}"
PERFORMANCE_QUERY_FILE="${PERFORMANCE_QUERY_FILE:-$CONVERTED_DIR/performance_query.i8bin}"
PERFORMANCE_QUERY_START="${PERFORMANCE_QUERY_START:-10000}"
PERFORMANCE_QUERY_END="${PERFORMANCE_QUERY_END:-20000}"
INSERT_FILE="${INSERT_FILE:-$CONVERTED_DIR/insert.i8bin}"
INSERT_VECTOR_START="${INSERT_VECTOR_START:-20000}"
INSERT_VECTOR_END="${INSERT_VECTOR_END:-29316}"
...
INDEX_PREFIX="${INDEX_PREFIX:-$INDEX_DIR/spacev100m_R${R}_bw${BUILD_BEAM}_${PARTITION_STRATEGY}_pq${PQ_SUBQUANTIZERS}}"
...
base_bin() { echo "$BENCHMARK_VECTOR_SOURCE"; }
query_bin() { echo "$CONVERTED_DIR/recall_query.i8bin"; }
groundtruth_bin() { echo "$CONVERTED_DIR/recall_groundtruth.bin"; }
...
ensure_built() {
  ...
  if grep -q '^DVSTOR_STORAGE_NODE_ONLY:BOOL=ON$' "$BUILD_DIR/CMakeCache.txt"; then
    ...
    for target in "$@"; do
      case "$target" in
        dvstor_compute_node|dvstor_breakdown_benchmark|dvstor_sift101m_long_insert_recall|dvstor_gpunetio_*)
          echo "target $target is unavailable because BUILD_DIR is storage-only: $BUILD_DIR" >&2
          echo "configure a separate build with -DDVSTOR_STORAGE_NODE_ONLY=OFF and set BUILD_DIR to it" >&2
          return 1
          ;;
      esac
    done
