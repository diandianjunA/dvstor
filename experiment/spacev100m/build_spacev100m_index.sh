...
for flag in RUN_INDEX_SELF_TESTS REBUILD_GRAPH REBUILD_PQ REBUILD_EXTENT; do
...
if [[ "${VALIDATE_ONLY:-0}" == 1 ]]; then
  ...
  if [[ -s "$METADATA_FILE" ]]; then
    ...
    echo "[validate] current metadata schema: $schema"
  elif [[ -s "$GRAPH_METADATA_FILE" ]]; then
    echo "[validate] schema-15 recovery metadata is present: $GRAPH_METADATA_FILE"
  else
    echo "[validate] no committed index metadata; next stage would be graph"
  fi
  print_commands
  exit 0
fi
...
if [[ "${OVERWRITE_INDEX:-0}" == 1 && "$REBUILD_GRAPH" != 1 ]]; then
...
{
  echo "[state] dataset=SPACEV100M stage=$stage prefix=$INDEX_PREFIX"
