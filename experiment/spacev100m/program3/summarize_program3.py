...
for row in rows:
    ...
    if row["kind"] == "motivation":
        ...
        performance[mode] = {
            ...
            "_gpu_prepare_us": float(g.get("average_gpu_prepare_us", 0)),
            "_gpu_rdma_issue_us": float(g.get("average_gpu_rdma_issue_us", 0)),
            "_gpu_graph_validation_us": float(g.get("average_gpu_graph_validation_us", 0)),
            "_gpu_neighbor_decode_us": float(g.get("average_gpu_neighbor_decode_us", 0)),
            "_gpu_pq_score_us": float(g.get("average_gpu_pq_score_us", 0)),
            "_gpu_visited_us": float(g.get("average_gpu_visited_us", 0)),
            "_gpu_beam_selection_us": float(g.get("average_gpu_beam_selection_us", 0)),
            "_gpu_merge_us": float(g.get("average_gpu_beam_merge_us", 0)),
            "_gpu_exact_us": float(g.get("average_gpu_exact_us", 0)),
            "_gpu_reported_other_us": float(g.get("average_gpu_other_us", 0)),
        }
...
for item in performance.values():
    # Keep the paper-facing decomposition intentionally coarse. RDMA retains
    # its original meaning (device time waiting for remote completion), so it
    # remains directly comparable with the previously reported 1154 us.
    item["gpu_candidate_maintenance_us"] = sum(item[key] for key in (
        "_gpu_graph_validation_us",
        "_gpu_beam_selection_us", "_gpu_neighbor_decode_us",
        "_gpu_visited_us", "_gpu_merge_us",
    ))
    item["candidate_maintenance_breakdown"] = {
        "Candidate Validation": item["_gpu_graph_validation_us"],
        "Candidate Select": (
            item["_gpu_beam_selection_us"] + item["_gpu_neighbor_decode_us"] +
            item["_gpu_visited_us"]
        ),
        "Merge": item["_gpu_merge_us"],
    }
    item["gpu_rdma_us"] = item["rdma_wait_us_per_query"]
    item["gpu_distance_us"] = item["_gpu_pq_score_us"] + item["_gpu_exact_us"]
    item["gpu_other_us"] = max(
        0.0,
        item["gpu_query_us"] - item["gpu_candidate_maintenance_us"] -
        item["gpu_rdma_us"] - item["gpu_distance_us"],
    )
    for key in [key for key in item if key.startswith("_")]:
        del item[key]
summary = {
    "experiment": "program3_motivation_then_effectiveness",
...
distance_share = late["gpu_distance_us"] / late["gpu_query_us"]
rdma_share = late["gpu_rdma_us"] / late["gpu_query_us"]
candidate_maintenance_share = (
    late["gpu_candidate_maintenance_us"] / late["gpu_query_us"]
)
merge_in_maintenance_share = (
    late["candidate_maintenance_breakdown"]["Merge"] /
...
lines = [
    ...
    "## 1. 实验设置与问题",
    "",
    "测试使用 SPACEV100M、Beam=128、Persistent GPU + GPUNetIO、Stable-Run 和 Live/DynaExtent。动机实验保持其他配置不变，只扫描每轮权威扩展批量 C=1/4/8/16；每点预热 5 s、测量 10 s。效果实验固定 C=16，只切换 Late-Issue 与 Exact-Early-Issue，每组预热 5 s、测量 20 s，均只运行一次。",
    "",
...
lines += [
    ...
    "|---|---:|---:|",
    f"| Candidate 维护 | {late['gpu_candidate_maintenance_us']:.1f} μs | {100*candidate_maintenance_share:.1f}% |",
    f"| RDMA | {late['gpu_rdma_us']:.1f} μs | {100*rdma_share:.1f}% |",
    f"| 距离计算 | {late['gpu_distance_us']:.1f} μs | {100*distance_share:.1f}% |",
    f"| Others | {late['gpu_other_us']:.1f} μs | {100*other_share:.1f}% |",
    f"| **合计** | **{late['gpu_query_us']:.1f} μs** | **100%** |",
    "",
    f"Candidate 维护占 GPU 查询时间的 **{100*candidate_maintenance_share:.1f}%**，是四类中最大的计算阶段；RDMA 等待占 **{100*rdma_share:.1f}%**。结合上一节中 Merge 随扩展批量增长的结果，可以看到完整 Merge 屏障把候选维护与 RDMA 等待串行起来；方案三解除的正是这条不必要依赖。",
    "",
    f"Candidate 维护的内部构成见 `program3_candidate_maintenance_breakdown.svg`。其中 Merge 占 **{100*merge_in_maintenance_share:.1f}%**，是 Candidate 维护中最大的单项开销。",
    "",
    "## 4. Prefix 占比与可重叠窗口",
    "",
